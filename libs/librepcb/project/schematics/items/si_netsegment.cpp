...
SI_NetSegment::SI_NetSegment(Schematic& schematic, const XmlDomElement& domElement) throw (Exception) :
    SI_Base(schematic), mUuid(), mNetSignal(nullptr)
{
    try
    {
        ...
        // Load all netpoints
        for (XmlDomElement* node = domElement.getFirstChild("netpoints/netpoint", true, false);
        ...
        // Load all netlines
        for (XmlDomElement* node = domElement.getFirstChild("netlines/netline", true, false);
        ...
        // Load all netlabels
        for (XmlDomElement* node = domElement.getFirstChild("netlabels/netlabel", true, false);
...
SI_NetSegment::SI_NetSegment(Schematic& schematic, NetSignal& signal) throw (Exception) :
...
void SI_NetSegment::setNetSignal(NetSignal& netsignal) throw (Exception)
{
...
void SI_NetSegment::addNetLabel(SI_NetLabel& netlabel) throw (Exception)
{
...
void SI_NetSegment::removeNetLabel(SI_NetLabel& netlabel) throw (Exception)
{
    if ((!isAddedToSchematic()) || (!mNetLabels.contains(&netlabel))) {
        throw LogicError(__FILE__, __LINE__);
    }
...
void SI_NetSegment::addToSchematic() throw (Exception)
{
    if (isAddedToSchematic()) {
        throw LogicError(__FILE__, __LINE__);
    }
...
void SI_NetSegment::removeFromSchematic() throw (Exception)
{
    if ((!isAddedToSchematic())) {
        throw LogicError(__FILE__, __LINE__);
    }
...
XmlDomElement* SI_NetSegment::serializeToXmlDomElement() const throw (Exception)
{
    if (!checkAttributesValidity()) throw LogicError(__FILE__, __LINE__);

    QScopedPointer<XmlDomElement> root(new XmlDomElement("netsegment"));
